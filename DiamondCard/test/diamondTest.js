...
describe('DiamondTest', async function () {
  ...
  it('should add CardFacet functions', async () => {
    const CardFacet = await ethers.getContractFactory('CardFacet')
    const cardFacet = await CardFacet.deploy()
    await cardFacet.deployed()
    addresses.push(cardFacet.address)
    const selectors = getSelectors(cardFacet).remove(['supportsInterface(bytes4)'])
    tx = await diamondCutFacet.diamondCut(
      [{
        facetAddress: cardFacet.address,
        action: FacetCutAction.Add,
        functionSelectors: selectors
      }],
      ethers.constants.AddressZero, '0x', { gasLimit: 800000 })
    ...
    result = await diamondLoupeFacet.facetFunctionSelectors(cardFacet.address)
    assert.sameMembers(result, selectors)
  })

  it('should CardFacet function call', async () => {
    const cardFacet = await ethers.getContractAt('CardFacet', diamondAddress)
    await cardFacet.createCard("title1", "url1")
    await cardFacet.createCard("title2", "url2")
    const cardCount = await cardFacet.getCardCounts();
